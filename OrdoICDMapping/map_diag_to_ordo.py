...
_HERE = Path(__file__).parent        # OrdoICDMapping/
_ROOT = _HERE.parent                 # data_query/
...
def main():
    ...
    parser.add_argument("--diag", default=str(_ROOT / "Data" / "diag.csv"), help="Path to diagnosis CSV")
    parser.add_argument(
        "--mapping",
        default=str(_HERE / "ordo_icd10_icd9_mapping.csv"),
        help="Path to ORDO ICD10 ICD9 mapping CSV",
    )
    parser.add_argument(
        "--output",
        default=str(_HERE / "diag_ordo_mapped.csv"),
        help="Output CSV path",
    )
    parser.add_argument(
        "--subtype-count-output",
        default=str(_HERE / "ordo_patient_counts_subtype.csv"),
        help="Output CSV path for counts by raw ordo_code",
    )
    parser.add_argument(
        "--parent-count-output",
        default=str(_HERE / "ordo_patient_counts_parent.csv"),
